...
all:
	$(CXX) -DA_EXPORTS -I. -stdlib=libc++ -Wall -Wextra -Werror -Wunreachable-code -Wunused -Winconsistent-missing-override -Wloop-analysis -O3 -DNDEBUG -arch arm64 -isysroot $(SDK) -fPIC -std=c++2a -o A.cpp.o -c A.cpp
	...
	$(CXX) -Dcircuit_EXPORTS -I. -isystem $(PYBIND_INC) -isystem $(PYROOT)/include/python3.8 -Wall -Wextra -Werror -O3 -DNDEBUG  -arch arm64 -isysroot $(SDK113) -fPIC -fvisibility=hidden -Os -Wno-register -Wno-deprecated-register -std=c++2a -MD -MT binder.cpp.o -MF binder.cpp.o.d -o binder.cpp.o -c binder.cpp
